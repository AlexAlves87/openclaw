...
public class WebSocketClientBaseTests
{
    ...
    {
        // Find a free port, close it immediately so the connection always refuses —
        // avoids false success when a real gateway happens to be running on the default port.
        int closedPort;
        ...
        var client = new TestWebSocketClient($"ws://localhost:{closedPort}", "token", _logger);
        var statuses = new List<ConnectionStatus>();
        client.StatusChanged += (_, s) => statuses.Add(s);

        // ConnectAsync will fail (port closed) but should still fire Connecting then Error
