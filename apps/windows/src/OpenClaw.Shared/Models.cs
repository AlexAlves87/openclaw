...
public class SessionInfo
{
    ...
    {
        if (n >= 1_000_000) return (n / 1_000_000.0).ToString("F1", System.Globalization.CultureInfo.InvariantCulture) + "M";
        if (n >= 1_000) return (n / 1_000.0).ToString("F1", System.Globalization.CultureInfo.InvariantCulture) + "K";
        return n.ToString();
    }
}
...
public class GatewayUsageInfo
{
    ...
    public string DisplayText
    {
        get
        {
            ...
            if (CostUsd > 0)
                parts.Add($"${CostUsd.ToString("F2", System.Globalization.CultureInfo.InvariantCulture)}");
            if (RequestCount > 0)
